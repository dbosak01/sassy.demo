...
#### Description: Simple Demographics Table with p-values.
####
####              Using upcoming "procs" package.
####              Plan to include frequently used statistical procedures. 
####              Functions mimic SAS quite closely.  Statistical output will match SAS.
...
proc_freq(adsl, tables = ARM, 
          options = v(out, long, nopercent, nonobs)) -> arm_pop
...
proc_means(adsl, var = AGE,
           ...
           by = ARM, 
           options = v(out, notype, nofreq)) -> age_stats 
...
proc_freq(adsl, tables = v(SEX * ARM),
          options = v(out, chisq, notable)) -> sex_chisq
...
proc_freq(adsl, tables = RACE,
          by = ARM,
          options = v(out, nonobs)) -> race_freq 
...
proc_freq(adsl, tables = RACE * ARM,
          options = v(out, chisq, notable)) -> race_chisq 
...
proc_freq(adsl,
          table = AGECAT,
          by = ARM,
          options = v(out, nonobs)) -> ageg_freq
...
proc_freq(adsl, tables = AGECAT * ARM,
          options = v(out, chisq, notable)) -> ageg_chisq 
...
rpt <- create_report("./output/ProcsDemoDM.rtf", 
                     output_type = "RTF", 
                     font = "Times") |> 
